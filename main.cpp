...
double U_rect(double x) {
    ...
    if (x >= left && x <= right) return -P.depth;
    return 0.0;
}

double U_parabolic(double x) {
    return P.k * pow(x - P.center, 2.0) - P.depth;
}
...
double U_triangular(double x) {
    ...
    double mid = P.center;
    if (x <= mid) return -P.depth * (x - left) / (mid - left);
    return -P.depth * (right - x) / (right - mid);
}

bool read_potential_from_file(const string &filename, vector<double> &xs, vector<double> &Us) {
...
double interp_linear(const vector<double> &xs, const vector<double> &Us, double x) {
    if (xs.empty()) return 0.0;
    if (x <= xs.front()) return Us.front();
    if (x >= xs.back()) return Us.back();
...
int main() {
    ...
    PotentialType type;
    switch (choice) {
        case 1: type = PotentialType::RECT_WELL; break;
        case 2: type = PotentialType::PARABOLIC; break;
        case 3: type = PotentialType::TRIANGULAR; break;
        case 4: type = PotentialType::FROM_FILE; break;
        default: type = PotentialType::RECT_WELL;
    }
    ...
    if (type == PotentialType::FROM_FILE) {
        have_file = read_potential_from_file(potential_input_file, xs_file, Us_file);
        if (!have_file) {
            cerr << "Ошибка: файл " << potential_input_file << " не найден или пуст.\n";
            return 1;
        }
    }
    ...
    for (int i = 0; i < N; ++i) {
        double x = (i + 1) * dx;
        switch (type) {
            case PotentialType::RECT_WELL: Uvec(i) = U_rect(x); break;
            case PotentialType::PARABOLIC: Uvec(i) = U_parabolic(x); break;
            case PotentialType::TRIANGULAR: Uvec(i) = U_triangular(x); break;
            case PotentialType::CUSTOM: Uvec(i) = U_custom(x); break;
            case PotentialType::FROM_FILE: Uvec(i) = interp_linear(xs_file, Us_file, x); break;
        }
    }
    ...
    fw << "# x (нм)\tU(x)\t";
    for (int n = 0; n < nsave; ++n) fw << "E" << n+1 << "\tpsi" << n+1 << "+E\t";
    fw << "\n";
    ...
    gp << "set title sprintf('Собственные функции и уровни в ящике [0, %.2f] нм', " << a << ")\n";
    gp << "set xlabel 'x, нм'\n";
    ...
    for (int n = 0; n < nsave; ++n) {
        ...
        stringstream title;
        title << "E_" << (n+1) << " = " << fixed << setprecision(3) << E(n) << " эВ";
    
        gp << ", \\\n";
        gp << " '' u 1:" << col_psi
           << " w l lc rgb '" << color
           << "' lw 2 title '" << title.str() << "'";
        gp << ", \\\n";
        gp << " '' u 1:" << col_E
           << " w l dt 2 lc rgb '" << color
           << "' lw 1.2 notitle";
    }
